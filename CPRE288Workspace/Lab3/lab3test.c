...
int main (void) {
    ...
    if (numObjects > 0) {
        int targetAngle = smallestObjectDegrees(objects, numObjects);
        cyBOT_Scan_t scan;
        cyBOT_Scan(targetAngle, &scan);

        char message[60];
        sprintf(message, "\r\nSmallest Object at %d degrees, Distance: %.2f\r\n", targetAngle, scan.sound_dist);
